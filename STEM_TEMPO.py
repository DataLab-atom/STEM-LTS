...
class STEM_TEMPO(nn.Module):
    
    def __init__(self, configs, device):
        super(STEM_TEMPO, self).__init__()
        ...
        if configs.is_gpt:
            if configs.pretrain:
                self.gpt2_trend = GPT2Model.from_pretrained('./models/gpt2', output_attentions=True,output_hidden_states=True)  # loads a pretrained GPT-2 base model  
            ...
            self.gpt2_trend_token = self.tokenizer(text="Predict the future time step given the trend", return_tensors="pt")
            self.gpt2_season_token = self.tokenizer(text="Predict the future time step given the season", return_tensors="pt")
            self.gpt2_residual_token = self.tokenizer(text="Predict the future time step given the residual", return_tensors="pt")
            self.gpt2_trend_token_input_ids = self.gpt2_trend_token['input_ids'].cuda()
            self.gpt2_season_token_input_ids = self.gpt2_season_token['input_ids'].cuda()
            self.gpt2_residual_token_input_ids = self.gpt2_residual_token['input_ids'].cuda()

            self.token_len = len(self.gpt2_trend_token_input_ids[0])
    ...
    def get_emb(self, x, tokens=None, type = 'Trend'):
        if tokens is None:
            ...
            if self.training:
                self.clip_loss += self.clip_criterion(prompt_x, x,self.logit_scale.exp())
    ...
    def forward(self, x, itr, trend, season, noise, test=False):
        ...
        if self.is_gpt and self.prompt == 1:
            if self.pool:
                trend, reduce_sim_trend, trend_selected_prompts = self.get_emb(trend, self.gpt2_trend_token_input_ids, 'Trend')
            else:
                trend = self.get_emb(trend, self.gpt2_trend_token_input_ids.to(trend.device), 'Trend')
        else:
        ...
        if self.is_gpt and self.prompt == 1:
            if self.pool:
                season, reduce_sim_season, season_selected_prompts = self.get_emb(season, self.gpt2_season_token_input_ids, 'Season')
            else:
                season = self.get_emb(season, self.gpt2_season_token_input_ids.to(trend.device), 'Season')
        else:
        ...
        if self.is_gpt and self.prompt == 1:
            if self.pool:
                noise, reduce_sim_noise, noise_selected_prompts = self.get_emb(noise, self.gpt2_residual_token_input_ids, 'Residual')
            else:
                noise = self.get_emb(noise, self.gpt2_residual_token_input_ids.to(trend.device), 'Residual')
        else:
        ...
        x_all = torch.cat((trend, season, noise), dim=1)#64,153,768

        x = self.gpt2_trend(inputs_embeds = x_all).last_hidden_state # 64,153,768
        ...
        STL_fature = torch.cat((trend,season,noise),-1).permute(0, 2, 1)
        self.STL_loss = (sum([torch.tril(torch.cov(i), diagonal=-1).mean().exp() for i in STL_fature])/float(STL_fature.shape[0])).log()
        if torch.isnan(self.STL_loss) or torch.isinf(self.STL_loss):
            torch.save(STL_fature.cpu(),'1_data.pt')
            return None
        ...
        if self.training:
            return outputs, loss_local,self.clip_loss,self.STL_loss
        else:
